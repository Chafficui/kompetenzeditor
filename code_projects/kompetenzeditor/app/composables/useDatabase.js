...
const db = new Dexie('kompetenzDB')

db.version(1).stores({
  verben: '++id, stufe, verb',
  schlechteVerben: '++id, verb',
  modalVerben: '++id, verb',
  einleitungen: '++id, *person, *adjektiv, *teilnahme, *modul',
  kompetenzbeschreibungen: '++id, name, text, saetze, wertung, datum, datumUnix',
})
  ...
  db.einleitungen.bulkAdd([
    {
      ...
      adjektiv: 'erfolgreich, positiv, optimal, bestmöglich, effizient, wirksam, angemessen',
      teilnahme: 'Teilnahme, Engagement, Aktivität, Beteiligung',
      modul: 'Betriebswirtschaftslehre 1, Digitaltechnik und Rechnersysteme, Mathematische Grundlagen der Informatik, Programmierung 1, Programmiermethoden und -werkzeuge, Technische Grundlagen der Informatik',
    },
  ])
...
export function useDatabase() {
  async function getAllKompetenzbeschreibungen() {
    return await db.kompetenzbeschreibungen.toArray()
  }

  async function getKompetenzbeschreibung(id) {
    return await db.kompetenzbeschreibungen.get(id)
  }

  async function saveKompetenzbeschreibung(id, data) {
    if (id > 0) {
      await db.kompetenzbeschreibungen.update(id, data)
      return id
    } else {
      return await db.kompetenzbeschreibungen.add(data)
    }
  }

  async function deleteKompetenzbeschreibung(id) {
    return await db.kompetenzbeschreibungen.delete(id)
  }

  async function getGoodVerbs() {
    return await db.verben.toArray()
  }

  async function getBadVerbs() {
    return await db.schlechteVerben.toArray()
  }

  async function getModalVerbs() {
    return await db.modalVerben.toArray()
  }

  return {
    db,
    getAllKompetenzbeschreibungen,
    getKompetenzbeschreibung,
    saveKompetenzbeschreibung,
    deleteKompetenzbeschreibung,
    getGoodVerbs,
    getBadVerbs,
    getModalVerbs,
  }
